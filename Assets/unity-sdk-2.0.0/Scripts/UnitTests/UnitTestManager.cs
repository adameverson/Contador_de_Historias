...
namespace IBM.Watson.DeveloperCloud.Editor
{
    ...
    public class UnitTestManager : MonoBehaviour
    {
        ...
        private Type _testsAvailable2 = null;
        ...
        public static TestTextToSpeech alterText;
        ...
        private void OnGUI()
        {
            if (_testsAvailable == null)
            {
                ...
                foreach (var t in Utility.FindAllDerivedTypes(typeof(MonoBehaviour)))
                {
                    if (t.Name.Equals("ExampleStreaming"))
                        _testsAvailable2 = t;
                }
                stringToEdit = "<prosody pitch=\"+0st\" rate=\"-0%\" volume=\"92\"> Oi, que tal uma aventura! </prosody>";
            }

            if (_testsAvailable != null)
            {
                GUILayout.BeginArea(new Rect(Screen.width * 0.0f, Screen.height * 0.0f, Screen.width * 1.0f, Screen.height * 0.85f));
                //GUILayout.BeginArea(new Rect(Screen.width * 0.3f, Screen.height * 0.76f, Screen.width * 0.4f, Screen.height * 0.85f));
                //GUILayout.BeginArea(new Rect(Screen.width * 0.375f, Screen.height * 0.825f, Screen.width * 0.25f, Screen.height * 0.85f));
                stringToEdit = GUILayout.TextArea(stringToEdit, 10000);
                alterText = new TestTextToSpeech(stringToEdit);

                foreach (var t in _testsAvailable)
                {
                    //string sButtonLabel = "Run " + t.Name;
                    string sButtonLabel = "Run TTS";
                    ...
                    /*
                    sButtonLabel = "Run STT";
                    if (GUILayout.Button(sButtonLabel, GUILayout.MinWidth(Screen.width * 0.4f), GUILayout.MinHeight(Screen.height * 0.04f)))
                    {
                        IBM.Watson.DeveloperCloud.Editor.UnitTestManager.ProjectToTest = _projectNameToTest;
                        QueueTest(_testsAvailable2, true);
                    }
                    */
                }
                GUILayout.EndArea();
