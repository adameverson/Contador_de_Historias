...
//		CreateShape ();
	...
	void Update ()
	{
		...
		if (Input.GetMouseButtonDown (0)) {
			if (!shape.completed) {
				var em1 = brightEffect.GetComponent<ParticleSystem> ().emission;
                em1.enabled = true;
			}
			...
			var em2 = brightEffect.GetComponent<ParticleSystem> ().emission;
            em2.enabled = false;
			DisableHand ();
...
//		TR_Path [] paths = GameObject.FindObjectsOfType<TR_Path> ();
	...
	{
		DisableHand ();
		var em3 = brightEffect.GetComponent<ParticleSystem> ().emission;
        em3.enabled = false;
